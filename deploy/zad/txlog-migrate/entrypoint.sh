#!/bin/sh
# Wrapper-entrypoint: migreer de DB, start dan de txlog-api. ZAD ondersteunt (nog)
...
# golang-migrate is idempotent + lockt; ga uit van 1 txlog-replica per peer.
echo "txlog-migrate: migraties draaien..."
/usr/local/bin/txlog-api migrate up --postgres-dsn "$DSN"

echo "txlog-migrate: serve starten..."
exec /usr/local/bin/txlog-api serve
