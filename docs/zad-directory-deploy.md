...
| `dirui` | `docker.io/federatedserviceconnectivity/directory-ui:v2.6.0` | Web (edge) + 3 bijlagen | dienstencatalogus-UI |
...
Draai `build-manager-migrate.yml` (Actions → workflow_dispatch, `image_tag=v2.6.0`), of merge een
wrapper-wijziging naar `main`. Resultaat: `ghcr.io/minbzk/moza-fsc-testnet-manager-migrate:v2.6.0`
(en/of `…:v2.6.0-pr-<n>` voor previews — de `image_suffix`-input van de reusable
...
`deployment=test`, `image_tag=v2.6.0`, `manager_tag=""` (canonieke tag).
...
| `build` | alleen als `manager-migrate/**` óf `build-manager-migrate.yml` zelf non-docs wijzigde | roept `build-manager-migrate` aan (reusable `workflow_call`); bouwt+pusht `v2.6.0` op main, `v2.6.0-pr-<n>` op een PR |
...
(`v2.6.0-pr-<n>`) uit die call, main de canonieke `v2.6.0`.
